...
async function fetchWithTimeout(url: string, timeoutMs: number = 5000) {
    ...
    try {
        ...
        if (error.name === 'AbortError') {
            throw new Error(`API request timed out after ${timeoutMs}ms. The GitHub API is currently too slow.`);
        }
        throw error; 
    }
}
...
async function analyzeRepo(repoPath: string) {
    ...
    try {
        ...
        console.log(` Open Issues:  ${repoData.open_issues_count.toLocaleString()}`);
        ...
        if (commitsData && commitsData.length > 0) {
            ...
            const oldestCommitDate = new Date(commitsData[commitsData.length - 1].commit.author.date);
            
       
            const timeSpanDays = Math.max(1, (latestCommitDate.getTime() - oldestCommitDate.getTime()) / (1000 * 3600 * 24));
            ...
            console.log(`Latest commit:   ${latestCommitDate.toISOString().split('T')[0]}`);
        }
...
function validateRepoInput(input: string | undefined): string {
    if (!input) {
    ...
    if (!repoRegex.test(input)) {
        console.error(` Error: Invalid input format "${input}". Expected format: <owner>/<repo> (e.g., facebook/react)`);
        process.exit(1);
    }
